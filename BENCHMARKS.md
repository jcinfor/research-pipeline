...
| mem0 reproduced by m_flow (likely v3 given timing) | 50.4% | `gpt-5-mini` answer + `gpt-4o-mini` judge, top-K=30 (independent third-party reproduction; their `Mem0 Cloud (tested)` row of 50.4%). Mem0 Cloud's hosted backend was running mem0's current production algorithm at m_flow's test date — that's v3 if the run post-dates April 2026, v2 if it pre-dates. Either way the takeaway holds: an independent reproducer with GPT-class infrastructure got significantly below mem0's self-published number. |
...
Two independent reproducers (m_flow + us) each measure mem0 v3 well below mem0's self-published number, *even with comparable algorithm + dep state*. The reproduction-protocol gap (m_flow's 50% with `gpt-5-mini` vs published 92%) is ~42pp; the further model-swap gap (m_flow's 50% with GPT-class to our 42-43% on Gemma 26B) is only 7-8pp.
